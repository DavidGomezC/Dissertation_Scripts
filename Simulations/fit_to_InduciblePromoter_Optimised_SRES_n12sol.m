function [out]=fit_to_InduciblePromoter_Optimised_SRES_n12sol(epccOutputResultFileNameBase,epccNumLoops,stepd,epcc_exps,global_theta_guess)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    results_folder = strcat('InduciblePromoterSRESn12sol',datestr(now,'yyyy-mm-dd-HHMMSS'));
...
for i=1:numLoops
    ...
    inputs.exps.tf_type{iexp}='fixed';
    ...
    inputs.exps.n_steps{iexp}=15;         % Also used round(duration/stepDuration); Number of steps in the input
    ...
    % OPTIMIZATION
    inputs.nlpsol.nlpsolver='hyb_sres_nl2sol';                                           % Differential evolution
    inputs.nlpsol.SRES.NP = max([200, 10*(2*inputs.exps.n_steps{iexp}-1)]);       % NP is the number of population members, usually greater than 10*number of decision variables
    inputs.nlpsol.SRES.itermax = round((300*1e3)/inputs.nlpsol.DE.NP);        % maximum number of iterations ('generations')
    inputs.nlpsol.SRES.cvarmax = 1e-5;                                        % cvarmax: maximum variance for a population at convergence
    inputs.nlpsol.SRES.varphi = 1;
    inputs.nlpsol.SRES.pf = 0.45;
...
save([strcat(epccOutputResultFileNameBase),'.mat'], 'oed_results','exps','inputs','true_param_values','best_global_theta_log');
