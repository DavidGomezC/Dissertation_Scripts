function [out]=fit_to_InduciblePromoter_Optimised_DE_n12sol_1(epccOutputResultFileNameBase,epccNumLoops,stepd,epcc_exps,global_theta_guess)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    results_folder = strcat('InduciblePromoterDEn12sol1',datestr(now,'yyyy-mm-dd-HHMMSS'));
...
for i=1:numLoops
    ...
    % OPTIMIZATION
    inputs.nlpsol.nlpsolver='hyb_de_nl2sol';                                           % Differential evolution
    inputs.nlpsol.DE.NP = max([200, 10*(2*inputs.exps.n_steps{iexp}-1)]);       % NP is the number of population members, usually greater than 10*number of decision variables
    inputs.nlpsol.DE.itermax = round((500*1e3)/inputs.nlpsol.DE.NP);        % maximum number of iterations ('generations')
    inputs.nlpsol.DE.cvarmax = 1e-5;                                        % cvarmax: maximum variance for a population at convergence
    inputs.nlpsol.DE.F = 0.85;                                               % F: DE-stepsize [0,2]
